...
const ShoppingTableRow = ({ item, idx }) => {
    if(!item) {
        return null;
    }
    ...
    return (
        <tr>
            ...
            <td className="ps-1 pe-1">{ item.userId }</td>
            <td className="ps-1 pe-1">{ item.subscription && item.subscription.name }</td>
            <td className="ps-1 pe-1">{ item.price }</td>
        </tr>
    );
}
